...
$subsList = db('Grupo', 'q', 'SELECT * FROM `gr_subs`', array());

$subUsers = db('Grupo', 'q', 'SELECT * FROM `gr_users` WHERE subs != 0', array());

$giftSounds = db('Grupo', 'q', 'SELECT gr_gifts.image,gr_gift_sound.sound FROM `gr_gifts` INNER JOIN gr_gift_sound ON gr_gifts.sound = gr_gift_sound.id WHERE gr_gifts.sound != 0', array());
...
<head>
    ...
    <meta http-equiv="origin-trial" content="AqcaILd5NewOX+LmgVrMkPtnMxVzVYm7f5WlD+jbvcIY9XAP9JpMdUqpYfy5pgN9/hF7uFEvxWGsTgkrax1l6wgAAABkeyJvcmlnaW4iOiJodHRwczovL2JhYmJlbG8uY29tOjQ0MyIsImZlYXR1cmUiOiJEaWdpdGFsR29vZHMiLCJleHBpcnkiOjE2MzE2NjM5OTksImlzU3ViZG9tYWluIjp0cnVlfQ==">
    ...
    <style type="text/css">
    <?php 
     $styles = db('Grupo', 'q', 'SELECT * FROM `gr_channels_styles`', array());
     foreach ($styles as $key => $value) {
    ?>

    <?php if ($value['background'] && $value['background'] !='0'): ?>
        .swr-grupo .panel > .room[t_id='<?php echo $value['channel']; ?>'] {
            background-image: url(https://sabayachat.com/chat/extra/backgrounds/<?php echo $value['background']; ?>);
        }  
    <?php endif ?>
   

    .swr-grupo .aside > .content > .list > li[no='<?php echo $value['channel']; ?>'] > div > .center > b > span {
        color: <?php echo $value['color']; ?>!important;
    }
    ...
    <script type="text/javascript">
        <?php 
        $prlang1 = $GLOBALS["default"]->language;
        if (isset($_COOKIE["grupolang"]) && !empty(vc($_COOKIE['grupolang'], "num"))) {
            $prlang1 = $_COOKIE["grupolang"];
        }
        echo 'var prlang = "'.$prlang1.'"';
        ?>
    </script>
</head>
        ...
        <div class='window fh'>
            <div class="container-fluid fh">
                <div class="row fh">
                            ...
                            <div class="tabs">
                                <ul>
                                    ...
                                    if (gr_role('access', 'files', '5')) {
                                        ?>
                                        <li act='files' <?php if (gr_role('access', 'files', '1')) { gec('class=uploadable'); } ?> style='display: none;' side='lside' zero='0KB' zval='<?php gec($GLOBALS["lang"]->zero_files) ?>'><?php gec($GLOBALS["lang"]->files) ?></li>
                                        <?php
                                    } ?>
                                    <?php
                                    ...
                                    <li act='lastSenders' side='lside' zero='0' unread='0' zval='...'>Today<i></i></li>
                                    <li act='mostPopuler' side='lside' zero='0' unread='0' zval='...'>Famous<i></i></li>
                    ...
                    <div class="<?php gec($GLOBALS["grusrlog"]['panelclass'].' '); ?>nomob panel" no=0 ldt="group" lstseen=0 deactiv=0>
                    ...
                    <?php if ($GLOBALS["logged"]) {
                        ...
                        <div class="col-md-5 col-lg-3 nomob aside rside tabfold">
                                ...
                                <span class='left'>
                                    ...
                                    <span class="vwp" no="<?php gec($GLOBALS["user"]['id']); ?>">
                                        ...
                                        <span><?php gec(gr_profile('get', $GLOBALS["user"]['id'], 'name')); ?>
                                            <?php usr('Grupo', 'flag', $GLOBALS["user"]['id']); ?>
                                            <span id='my_memberships' title="@<?php gec(usr('Grupo', 'select', $GLOBALS["user"]['id'])['name']); ?>"></span>
                                        </span>
                                    </span></span>
                                <span class='right'>
                                    ...
                                    <?php if (isset($GLOBALS["roles"]['groups'][16])) {
                                        ...
                                        <li act='crew' class='grtab d-none' zero='0' zval='<?php gec($GLOBALS["lang"]->zero_crew) ?>' side='rside'><?php gec($GLOBALS["lang"]->crew) ?>
                                            <ul class='subtab'>
                                                ...
                                                <li filtr='alalll'><?php gec($GLOBALS["lang"]->all) ?></li>
                            ...
                            <div class="content">
                                ...
                                <div class="profile">
                                    <div class="top">
                                        <span id="reportUser"><span class="gi-exclamation"></span> Report This User</span>
...
<link href="<?php gec($GLOBALS["default"]->weburl) ?>gem/tone/custom.css?t=<?php echo time();?>" rel="stylesheet">
<?php
gr_cbg();
?>

<script type="text/javascript">
    var home_url = 'https://sabayachat.com/chat/';
</script>
...
<script type="text/javascript">AgoraRTC.disableLogUpload()</script>
...
<script src="<?php gec($GLOBALS["default"]->weburl) ?>gem/mine/gr-live.js?t=<?php echo time();?>"></script>
<script src="<?php gec($GLOBALS["default"]->weburl) ?>gem/mine/grupomine.js?t=<?php echo time();?>"></script>

<script src="<?php gec($GLOBALS["default"]->weburl) ?>extra/assets/i18n/jquery.i18n.js"></script>
<script src="<?php gec($GLOBALS["default"]->weburl) ?>extra/assets/i18n/jquery.i18n.messagestore.js"></script>


<script src="<?php gec($GLOBALS["default"]->weburl) ?>extra/payment.js?t=<?php echo time();?>"></script>
<script src="<?php gec($GLOBALS["default"]->weburl) ?>extra/addons.js?t=<?php echo time();?>"></script>

<script type="text/javascript">
    
    var subsNames = [];
    <?php foreach ($subsList as $key => $value): ?>
    subsNames[<?php echo $value['id']; ?>] = '<?php echo $value['name']; ?>';
    <?php endforeach ?>
    var sUsers = [] ;
    <?php foreach ($subUsers as $key => $value): ?>
    sUsers[<?php echo $value['id']; ?>] = <?php echo $value['subs']; ?>;    
    <?php endforeach ?>
    
    var giftSounds = <?php echo json_encode($giftSounds); ?>;
