...
var reb = new RegExp('(?:[\b]|[\u0600-\u06FF]|^)'+badWords[i]+'(?=[\b]|$)', 'gmi');
...
strtest= nodes[x].nodeValue.replace(/[&\/\\#,+()\[\]'=;|$~%.'":*?<>{}\r?\n|\r]/gmi, '');  
if (re.test(strtest)) {
                        profane = true;
                        data.push(badWords[i]);
                        if (options.filter) {
var spiltwords = nodes[x].nodeValue.split(" ");
var index;
for (index = 0; index < spiltwords.length; ++index) {
spiltwords[index]= spiltwords[index].replace(/[&\/\\#,+()\[\]'=;|$~%.'":*?<>{}\r?\n|\r]/gmi, ''); 
                           spiltwords[index] = spiltwords[index].replace(reb, rep);
}
nodes[x].nodeValue = spiltwords.join(" ");
                        }
                    }
                }
                ...
                for (var x = 0; x < inputs.length; x++) {
                    if (re.test(inputs[x].value)) {
                        profane = true;
                        data.push(badWords[i]);
                        if (options.filter) {                            $(inputs[x]).val(inputs[x].value.replace(re, rep));
                        }
                    }
                }
            }
