...
var current_group = '', remoteHosts = {}, audioTrack = null, agora_uid = '', chlJoined = false;
agora_client.on("user-published", handleUserPublished);
agora_client.on("user-unpublished", handleUserUnpublished);
...
function loadlist(el, e) {
    ...
    s = s+'</span><span class=center><b><span data-toggle=tooltip title='+"'"+'"+htmlDecode(data[k].name)+"'+"'"+'>"+htmlDecode(data[k].name)+"</span></b><i class="+data[k].icon+"></i><i class="+data[k].icon1+"></i><i class="+data[k].flag1+" style="+data[k].flag+"></i>";';
    ...
    s = s+'list=list+"</u><span>"+htmlDecode(data[k].sub)+"</span>";';
    s = s+'if(data[k].subs && data[k].subs!==0){ list=list+subsBabge(data[k].subs); }';
    s = s+'list=list+"</span><span class=right>';
    s = s+'<span class=opt "+data[k].rtag+"><i class=gi-dot-3></i><ul>";';
    s = s+'if(data[k].og && data[k].og!==0){list=list+"<li "+data[k].ogt+">"+htmlDecode($.i18n(data[k].og))+"</li>";}'; 
    ...
    if (el.attr('act') === 'groups' || el.attr('act') === 'pm') {
        ...
        s = s+'$(".swr-grupo .'+el.attr('side')+' > .content > .list > li[no="+$(".swr-grupo .panel").attr("no")+"]").find("div > .center > u").html("");';
...
async function agora_connect(channel_name) {
    // console.log(channel_name);
    ajxvar['loadtoken'] = $.ajax({
        type: 'POST',
        url: '',
        data: {
            act: 1,
        ...
        dataType: 'json',
        beforeSend: function() {
            if (ajxvar['loadtoken'] !== null && ajxvar['loadtoken'] !== undefined) {
                ajxvar['loadtoken'].abort();
            }
        },
        success: function(data) {}
        ...
        if(resp.status) {
            if(resp.is_host) {
                [ agora_uid, audioTrack ] = await Promise.all([
                    agora_client.join(resp.appid, channel_name, resp.token),
                    AgoraRTC.createMicrophoneAudioTrack()
                ]);

                audioTrack.play("local-player");
                await agora_client.setClientRole('host');
                await agora_client.publish(audioTrack);
            } else {
                await Promise.all([
                    agora_client.join(resp.appid, channel_name, resp.token)
                ]);
...
$('body').on('click', '.swr-grupo .loadgroup,.dumb .loadgroup', async function(e) {
    $('.swr-grupo .aside > .content > .list > li').removeClass("active");
    $(this).addClass("active");
    $(this).find('div > .center > u').html('');
    const sel_group = $(this).attr('no');
    loadgroup(sel_group, $(this));
    // if(current_group != sel_group && ['7', '3415'].indexOf(sel_group) >= 0) {
    ...
    // if(current_group != sel_group && sel_group == "3415") {
    // if(sel_group == "3415") {
    if(current_group != sel_group) {
        current_group = sel_group;
        agora_connect('group-' + current_group);
    }
});
    ...
    if ($id != $('.swr-grupo .panel').attr('no') || $id == $('.swr-grupo .panel').attr('no') && e.attr('ldt') != $('.swr-grupo .panel').attr('ldt') || r == 1 || e.attr('msgload') != undefined) {
        ...
        if ($('#send_gift').length) {
           if ($('.swr-grupo .panel').attr('ldt')=='user') {
            $('#send_gift').show();
                giftTarChannel = 0;
                $('.swr-grupo .panel > .room').attr('t_id',0);
            } else {
                $('#send_gift').hide();
                giftTarChannel = e.attr('no');
                $('.swr-grupo .panel > .room').attr('t_id',giftTarChannel);
            } 
        }
            ...
            if (trn == 1) {
                ...
                if (m.type == 'like') {
                    ...
                    if (m.type == 'system') {
                        ...
                        if (n == 2) {
                            ...
                            if (m.domsg == 'renamed_group') {
                                ...
                                $(".swr-grupo .panel > .room > .groupreload > i").trigger("click");
                            } else if (['is_now_moderator', 'is_now_admin', 'is_no_longer_admin_moderator', 'is_now_host', 'is_no_longer_host'].indexOf(m.domsg) >= 0) {
                    ...
                    if (m.type == 'msg') {
                        ...
                        if (m.userid == 0) {
                            let actionData = m.msg.split(' ');
                            var targetGiftFile = 'gift.png';
                            if (typeof actionData[5] !== 'undefined')
                                 targetGiftFile =actionData[5];
                            bclass = 'system system_big';
                            
                            m.name = actionData[3];
                            if (actionData[1]=='gift'){

                                if (typeof m.tdif !== 'undefined' && parseInt(m.tdif) > 0 && parseInt(m.tdif) < 10) {
                                    $.each(giftSounds, function( index, value ) {
                                      if (value.image == targetGiftFile) {
                                        var alertElement = new Audio(home_url+'extra/sounds/'+value.sound);
                                        alertElement.play();
                                      }
                                    });
                                }

                                m.msg = '<img src="'+home_url+'/extra/gifts/'+targetGiftFile+'" class="addonSystemAlert">'+$.i18n('channel_gift_sent',actionData[4],actionData[2]);
                            }
                            else  if (actionData[1]=='credit')
                                m.msg = '<img src="'+home_url+'extra/gifts/coin.png" class="addonSystemAlert">'+$.i18n('channel_credit_sent',actionData[4],actionData[2]);

                        }



                        if (m.lntitle != 0 && m.msg.indexOf('#GIFT') == -1) {
                    ...
                    if (m.name != undefined) {
                        if (m.status == 'deactivated') {
                            mntz = '<i class="usrname">'+getSubIcon(m.userid,m.name)+'</i>';
                        } else {
                            mntz = '<i class="usrname vwp" style="color:'+m.ncolor+'" no="'+m.userid+'">'+getSubIcon(m.userid,m.name)+'</i>';
                        }
                    }
                    ...
                    if (m.msg.indexOf('#GIFT') == 0) {
                        ...
                        if (moset=='right'){
                            if (datas[1]!='credit')
                                msg = msg+'<span class="preview image gifs lrmbg"><span style="margin-top: 0;width:100%;"><span style="font-size:10px;display:inline-block; width:100%;text-align:center;">'+$.i18n('you_send_gift',datas[2])+'</span></br><img class="lazyimg tenor" gif="'+home_url+'extra/gifts/'+datas[1]+'" data-src="'+home_url+'extra/gifts/'+datas[1]+'" style="height:100px;"/></span></span>';
                            else {
                                msg = msg+'<span>'+$.i18n('you_send_credit',home_url,datas[2])+'</span></br>';
                            }
                        }
                        else{
                            if (datas[1]!='credit')
                                msg = msg+'<span class="preview image gifs lrmbg"><span style="margin-top: 0;width:100%;"><span style="font-size:10px;display:inline-block; width:100%;text-align:center;">'+$.i18n('you_receive_gift',datas[2])+'</span></br><img class="lazyimg tenor" gif="'+home_url+'extra/gifts/'+datas[1]+'" data-src="'+home_url+'extra/gifts/'+datas[1]+'" style="height:100px;"/></span></span>';
                            else {
                                msg = msg+'<span>'+$.i18n('you_receive_credit',home_url,datas[2])+'</span></br>';
                            }
                        }
                        msg = msg+mntz;
                        m.type = 'stickers';
    ...
    setTimeout(function() {
        ...
        ajxvar['loadinfo'] = $.ajax({
            ...
            if (data[0].name == undefined) {
                ...
                $(".swr-grupo .aside > .content .profile > .middle > span.userBabgeArea").html('');
                if (data[0].subs>0) {
                    $(".swr-grupo .aside > .content .profile > .middle > span.userBabgeArea").html(subsBabge(data[0].subs));
                }
...
function subsBabge (subs) {
    if (subs == 0)
        return '';
    return '<span class="userBabge">'+subsNames[parseInt(subs)]+'</span>';

}

function getSubIcon(user_id,nickname) {
    if (typeof sUsers[user_id] !== 'undefined') {
        return '<img class="subsIcon" style="max-height:19px;" src="'+home_url+'extra/assets/subs/'+sUsers[user_id]+'.png" />'+nickname;
    }
