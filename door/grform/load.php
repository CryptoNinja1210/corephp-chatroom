...
function gr_form($do) {
    ...
    if ($do["type"] == "creategroup") {
        ...
        if (isset($GLOBALS["roles"]['groups'][14])) {
            $fields->visibility = array($GLOBALS["lang"]->visibility, 'select', '0', '-----', '0', $GLOBALS["lang"]->visible, '1', $GLOBALS["lang"]->hidden,'2', 'Paid');
            $fields->credits = array('Credits per minute', 'input', 'text');
        }
        //end
        ...
        if (gr_role('access', 'groups', '2') && $adm == 1 || gr_role('access', 'groups', '7')) {
            ...
            if ($cr && count($cr) > 0) {
                ...
                if (isset($GLOBALS["roles"]['groups'][14])) {
                    $fields->visibility = array($GLOBALS["lang"]->visibility, 'select', '0', '-----', '0', $GLOBALS["lang"]->visible, '1', $GLOBALS["lang"]->hidden,'2', 'Paid');
                    $fields->credits = array('Credits per minute', 'input', 'text','"'.$cr['0']['v7'].'"');
        ...
        if ($cr && count($cr) > 0) {
            ...
            if (!$cu) {
                ...
                if (!empty($cr['0']['v3']) && !gr_role('access', 'groups', '7') && ($cr['0']['v3']=='paid' || $cr['0']['v3']=='secret') && floatval($cr['0']['v7'])>0) {
                    $msg1 = 'غرفة مسبوقة الدفع, ادخل عدد الدقايق التي تود ان تبقي فيها داخل الغرفة تكلفة الدقيقة '.$cr['0']['v7'].'  كوينز الرجاء تاكيد استعمال الكوينز, و التاكد من عدم مغادرة الغرفة قبل انتهاء المدة لكي لا تخسر الدقايق';
                    $fields->paidDescription = array('غرف مسبوقة الدفع', 'textarea', 'disabled', $msg1);
                    $fields->credits = array('ادخل عدد الدقايق', 'input', 'number');
                }
        ...
        if ($cr[0]) {
            ...
            $fields->id = array('hidden', 'input', 'hidden', '"'.$do["id"].'"');
            $fields->role = array($GLOBALS["lang"]->role, 'select', '0', '-----', '2', $GLOBALS["lang"]->admin, '10', $GLOBALS["lang"]->host, '11', $GLOBALS["lang"]->active, '12', $GLOBALS["lang"]->super, '1', $GLOBALS["lang"]->moderator, '0', $GLOBALS["lang"]->member);
