...
function fill_country($d, $userid) {
    $query = "SELECT * FROM `gr_profiles` WHERE uid=:userid AND name=6";
    $qdata = array();
    $qdata['userid'] = $userid;
    $_flag = db($d, 'q', $query, $qdata);
    ...
    if(count($_flag) === 0) {
        ...
        if($ipaddress != 'UNKNOWN' && strlen($ipaddress) > 0) {
            $url = "http://ip-api.com/php/" . $ipaddress . "?fields=status,country,countryCode";
            // $url = "http://ip-api.com/php/104.250.131.79?fields=status,country,countryCode";
            $ch = curl_init();
            ...
            if($_result['status'] === "success") {
                $query = "INSERT INTO `gr_profiles` (type, uid, name, v1, v2) VALUES('profile', '" . $userid . "', '6', '" . $_result['country'] . "', '" . $_result['countryCode'] . "')";
                db('Grupo', 'q', $query);
            }
        }
    }
...
function usr() {
    ...
    if ($t === 'register') {
        ...
        try {
            ...
            if (usr($d, 'exist', $userProfile->email)) {
                ...
                if(empty($userProfile->email)){
                    $userProfile->email=$r[4]."@sabayachat.com";
                }
        ...
        $i = strtolower(vc($arg[2], 'num'));
        $query = "SELECT * FROM gr_profiles WHERE uid = :uid AND type = 'profile' AND name = '6'";
        $data = array();
        $data['uid'] = $i;
        ...
        if(count($rz) > 0) {
            $rz = $rz[0];
            echo "<i style='background-image: url(https://api.hostip.info/images/flags/" . strtolower($rz['v2']) . ".gif);' class='host-mic1' alt='profile-image'></i>";
        }
    } else {
        return ses($d);
    }
